...
/*
								while(!myProxy_->atEnd()){
								TTree *tree = myProxy_->getTTree();
								Long64_t lastTreeEvent = tree->GetEntries();
								Long64_t globalLastTreeEvent = myProxy_->getTChain()->GetChainEntryNumber(lastTreeEvent);
								TTree::TClusterIterator clusterIter = tree->GetClusterIterator(0);

								Long64_t clusterStart, globalClusterStart;
								std::vector<Long64_t> clusterStartV;
								while( (clusterStart = clusterIter()) < tree->GetEntries() ) {
									globalClusterStart = myProxy_->getTChain()->GetChainEntryNumber(clusterStart);
									//std::cout<<"clusterStart: "<<clusterStart
									//<<" globalClusterStart: "<<globalClusterStart
									//<<std::endl;
									clusterStartV.push_back(globalClusterStart);
								}
								clusterStartV.push_back(globalLastTreeEvent);

								#pragma omp parallel for ordered schedule(dynamic,2)
								for(unsigned int iCluster = 0;iCluster<clusterStartV.size()-1;++iCluster){
									Long64_t clusterStart = clusterStartV[iCluster];
									Long64_t nextClusterStart = clusterStartV[iCluster+1];

									for(unsigned int aEvent=clusterStart; aEvent<nextClusterStart && aEvent<nEventsToAnalyze_; ++aEvent) {
								...
								myProxy_->toN(globalLastTreeEvent);
								clusterStartV.clear();
							}
							std::cout<<"globalLastTreeEvent: "<<globalLastTreeEvent
							         <<"tree: "<<myProxy_->getTTree()->GetCurrentFile()->GetName()
											 <<std::endl;
						}
*/

#pragma omp parallel for schedule(dynamic,1490)
