...
#pragma omp critical(ROOFIT_INITIALIZATION)
  {
    ...
    RooAbsReal *muon_id_iso_scalefactor = scaleWorkspace->function("m_idiso0p15_desy_ratio");
    RooAbsReal *muon_trg_scalefactor = scaleWorkspace->function("m_trgMu22OR_eta2p1_desy_ratio");   		                                                                   
    RooAbsReal *muon_xtrg_scalefactor_iso  = scaleWorkspace->function("m_trgMu19leg_eta2p1_desy_ratio");
    ...
    h2DMuonIdIsoCorrections = (TH2F*)muon_id_iso_scalefactor->createHistogram("h2DMuonIdCorrections",
									      *scaleWorkspace->var("m_pt"),RooFit::Binning(1980,10,1000),
									      RooFit::YVar(*scaleWorkspace->var("m_eta"),RooFit::Binning(480,-2.4,2.4)),//MB m_abs_eta->m_eta
    ...
    h3DMuonTrgCorrections = (TH3F*)muon_trg_scalefactor_proj->createHistogram("h3DMuonTrgCorrections",
									      *scaleWorkspace->var("m_pt"),RooFit::Binning(1980,10,1000),
									      RooFit::YVar(*scaleWorkspace->var("m_eta"),RooFit::Binning(480,-2.4,2.4)),
									      RooFit::ZVar(*scaleWorkspace->var("m_iso"),RooFit::Binning(120,-0.05,0.55)),
									      RooFit::Extended(kFALSE),
									      RooFit::Scaling(kFALSE));
    ...
    TH2F *h2DMuonXTrgCorrections_aiso = (TH2F*)muon_xtrg_scalefactor_iso->createHistogram("h2DMuonXTrgCorrections_aiso",
											  *scaleWorkspace->var("m_pt"),RooFit::Binning(1980,10,1000),
											  ...
											  RooFit::Extended(kFALSE),
											  RooFit::Scaling(kFALSE));
    h3DMuonXTrgCorrections = new TH3F("h3DMuonXTrgCorrections","",1980,10,1000,480,-2.4,2.4,80,-0.05,0.35);//less iso-bins than for id/single-trg due to different range
...
float ChannelSpecifics::getLeptonCorrection(float eta, float pt, float iso,
  ...
  if(!h2DMuonIdIsoCorrections) initializeLeptonCorrections();
  ...
  if(tauDecayMode == HTTAnalysis::tauDecayMuon) {
    ...
    iBin = h2DMuonIdIsoCorrections->FindBin(std::min(pt,(Float_t)999.9), eta);
    float id_isoweight = h2DMuonIdIsoCorrections->GetBinContent(iBin);

    iBin = h1DMuonTrkCorrections->FindBin(eta);
    float trackingweight = h1DMuonTrkCorrections->GetBinContent(iBin);

    return trigweight*id_isoweight*trackingweight;
  }
